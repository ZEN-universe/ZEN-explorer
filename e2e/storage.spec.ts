...
test('storage: default', async ({ page }) => {
  await page.goto('/energy_balance/storage');await page.getByLabel('Solution', { exact: true }).selectOption('european_electricity_heating_transition');
...
test('storage: carrier', async ({ page }) => {
  ...
  await expect(page.locator('#level_chart')).toHaveScreenshot(`eeht_pf/carrier-electricity/level.png`);
  await expect(page.locator('#flow_chart')).toHaveScreenshot(`eeht_pf/carrier-electricity/flow.png`);
});
...
test('storage: smoothing window size', async ({ page }) => {
  ...
  await expect(page.locator('#level_chart')).toHaveScreenshot(`eeht_pf/smoothing-window-size-monthly/level.png`);
  await expect(page.locator('#flow_chart')).toHaveScreenshot(`eeht_pf/smoothing-window-size-monthly/flow.png`);
});
...
test('storage: technology subdivision', async ({ page }) => {
  ...
  await expect(page.locator('#level_chart')).toHaveScreenshot(`eeht_pf/technology-subdivision/level.png`);
  await expect(page.locator('#flow_chart')).toHaveScreenshot(`eeht_pf/technology-subdivision/flow.png`);
});
...
test('storage: technologies', async ({ page }) => {
  ...
  await page.getByLabel('Subsolution').selectOption('perfect_foresight');
  await page.locator('div').filter({ hasText: /^Technologies Deselect all$/ }).getByRole('button').click();
